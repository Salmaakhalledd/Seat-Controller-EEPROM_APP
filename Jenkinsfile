pipeline {
    agent any

    stages {
        stage('Build') {
            steps {
                bat '''
                    if not exist out mkdir out
                    dir /s /b src\\*.java > sources.txt
                    javac -encoding UTF-8 -cp lib\\jSerialComm-2.11.2.jar -d out @sources.txt
                    jar cfe eeprom-app.jar eeprom.EepromGUI -C out .
                '''
            }
        }

        stage('Test') {
            steps {
                bat '''
                    dir /s /b eeprom\\*Test.java > test-sources.txt
                    javac -encoding UTF-8 -cp "lib\\jSerialComm-2.11.2.jar;lib\\junit-4.13.2.jar;lib\\hamcrest-core-1.3.jar;out" -d out @test-sources.txt
                    java -cp "lib\\jSerialComm-2.11.2.jar;lib\\junit-4.13.2.jar;lib\\hamcrest-core-1.3.jar;out" org.junit.runner.JUnitCore eeprom.NvmManagerTest
                '''
            }
        }

        stage('Build Docker Image') {
            when {
                expression { currentBuild.result == null || currentBuild.result == 'SUCCESS' }
            }
            steps {
                script {
                    docker.build("eeprom-app:latest")
                }
            }
        }

        stage('Run in Docker') {
            when {
                expression { currentBuild.result == null || currentBuild.result == 'SUCCESS' }
            }
            steps {
                script {
                    docker.image("eeprom-app:latest").run()
                }
            }
        }
    }
}
